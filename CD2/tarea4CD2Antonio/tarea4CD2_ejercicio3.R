...
polys <- poligonoRegion@polygons[[1]]@Polygons
...
plot(poligonoRegion, main='Peso por ubicaciÃ³n',
     xlim=c(-100.5, -100.3),
     ylim=c(25.61, 25.72))
points(casos@coords, col=chido, pch=20,
       cex=((casos@data$Peso)-min(casos@data$Peso))/max(casos@data$Peso)*6 )
...
########################
primer <- casos@coords[1,]
m1 <- get_map(location = primer, zoom=17,
              maptype="terrain",
              source="google")
ggmap(m1) + geom_point(data = as.data.frame(t(primer)),
                       aes(x=coords.x1, y=coords.x2,
                           colour=I(color3), size=2))+
