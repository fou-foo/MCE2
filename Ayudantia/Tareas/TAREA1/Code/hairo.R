...
for(i in seq(1,10,1)){ #generando los lanzamientos
  pmf[i]<-dunif(i,min = 0,max = 10, log = FALSE)# la funcion dunif ya esta vectorizada, no hacia falta el ciclo for
}
SampleSpace<-1:10
...
plot(SampleSpace,cdf, type = "b", main="CDF Uniforme Discreta", xlab = "Espacio Muestral",
     ylab = "Probabilidad Acumulada",ylim=c(0, 1)) #en la segunda ayudantia hablamos de como habia que construir esta grafica te mencione que estubieses al tanto
...
mean(uniform)
#La varianza es de:
var(uniform) # tu resultado tiene menor varianza porque sesgaste los resultados con el 'floor'
...
for (i in seq(1,1000000,1)){
  ...
  setTxtProgressBar(pb,i)#buen detalle, pero te sugiero usar las funciones mapply, lapply, etc en lugar de ciclos
}
...
#algo paso aqui porque deberia de ser muy parecidas
...
legend("topright", legend=c("Binomial", "Sample"), pch=c(1,10), 
       col=c("red", "black"),
       horiz=FALSE, bty='n', cex=0.9)
#igual el desface aqui te fallo
...
#parte importante de este ejercicio era construir tu propia funcion de quantiles
...
# lo curioso es que aqui sí te salio 