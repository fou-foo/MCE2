...
for (i in c(1:n)) {
  ...
  repeat {  # Se realiza el sample. Cuando sea 0 se detiene
    muestra <- sample(c(0, 1), 1, prob = c(p, 1-p)) # asignamos probabilidades
                                             # debo a que es una moneda sesgada y en la vida real QUE MONEDA ES JUSTA ?
    if (muestra == 1) {
...
#era la media de las realizaciones no de las probas me preocupa que confundas estos dos conceptos
mean(lanz1)
sd(lanz1)
mean(lanz2)
sd(lanz2)
...
# te doy .75 del ejercicio porque hiciste la parte dificil exepto parte importante del c) que 
# precizamente validaba tu simulacion 
