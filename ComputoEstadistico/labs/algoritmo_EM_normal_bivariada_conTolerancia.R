...
emiteration <- function(Y, p, R, n=1000) #quitamos el peligroso parametro '...'
{
  ...
  for (i in (3:n))
  {
      ...
      if( tolerancia > 1e-13) #fijamos una tolerancia como lo sugieren las notas
      {                         # una condicion fuerte
          p.old <- p.new
          p.new <- emstep(Y, p.old, R)
      } else {return(list(estimacion=p.old, iteraciones=i))}
  }
...
p <- emiteration(Y, p, n=1e8, R=R)
p$estimacion #estimaciones finales de los parametros
p$iteraciones #numero de veces que itero
#Check for convergence
#p<-emstep(Y,p,R=R)
