...
mult.outliers <- function(object, c.sig = 0.05){
  ...
  var.cov <- round(cov(resid.object), 4)
  resid.scale <- scale(resid(object), center = TRUE, FALSE)

  mat.out <- resid.scale%*%solve(cov(resid.scale))%*%t(resid.scale)

  test.out <- diag(mat.out)
  p.values <- 1 - matrix(pchisq(test.out, ncol(resid.object)))
  out.var  <- matrix(0, nrow = nrow(resid.object), ncol = 1)
  out.var[p.values < c.sig,1] <- 1
...
factor.pls <- function(y, x, xreg = NULL, c.var = 0.80){
  y <- as.matrix(y)
  x <- as.matrix(x)
  m.pls <- plsr(log(y) ~ scale(x))
  w.pls <- m.pls$Xvar/sum(m.pls$Xvar)
  w.pls <- w.pls[1: which(cumsum(w.pls) > c.var)[1]]
  w.pls <- w.pls/sum(w.pls)
  load.mat <- matrix(m.pls$loading.weights[,1:length(w.pls), drop = FALSE], ncol = length(w.pls))
  rownames(load.mat) <- colnames(x)
  score.pls <<- matrix(rowSums(sapply(1 : length(w.pls), function(x) m.pls$scores[, x, drop=FALSE] * w.pls[x])))
  n <- nrow(y)
  if(n > nrow(score.pls)){
    fore.score <- forecast(auto.arima(score.pls, xreg = xreg[1: nrow(score.pls), , drop = FALSE]),
                    newxreg = xreg[c(nrow(score.pls) + 1): n, , drop = FALSE],
                    h = n -  nrow(score.pls))$mean
    score.pls <<- matrix(c(score.pls, fore.score))
  }
  if(nrow(load.mat) > 1){
...
gon.gra <- function(x){
  ...
  ft <-  matrix(ft, N, p - r)
  colnames(ft) <- sapply(1 : ncol(ft), function(x)
                    paste("Common.factor.", x, sep=""))
  zt <- matrix(zt, N, r)
  colnames(zt) <- sapply(1 : ncol(zt), function(x)
                    paste("Cointegration.", x, sep=""))
