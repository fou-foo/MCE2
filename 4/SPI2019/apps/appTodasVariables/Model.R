...
importantes.categoricas <- c('Garantias.dCapExcitacion', 'Garantias.dCapEficReg',
                             ...
                             'iDestino')
####################################################################
setwd('/home/fou/Desktop/MCE2/4/SPI2019/apps/appTodasVariables')
dir()
...
datos$BoquillaAT.sDescripcionBoq <- factor(datos$BoquillaAT.sDescripcionBoq)
datos$BoquillaAT.sNoArticulo <- factor(datos$BoquillaAT.sNoArticulo )
...
arbol <- function(variable.predecir, data=datos)
{
    ...
    rfsrcFast(formula.foo, data=data, na.action="na.impute") # modelo
}
